...
internal object XrealXbxA01PlusDriver : GlassesDriver {
    ...
    override fun identify(device: UsbDevice): GlassesModel? = if (device.vendorId == 0x3318 && device.productId == profile.productId)
        GlassesModel(
            ...
            id,
            supportedDisplayProfiles = XrealXbxA01PlusDisplayModeProtocol.profiles,
            preferred2dDisplayProfile = XrealXbxA01PlusDisplayModeProtocol.profiles.firstOrNull {
                it.layout == GlassesDisplayLayout.MONO_2D && it.refreshRateHz == 60
            },
            preferred3dDisplayProfile = XrealXbxA01PlusDisplayModeProtocol.profiles.firstOrNull {
    ...
    override fun open(usbManager: UsbManager, device: UsbDevice, model: GlassesModel, feature: SessionFeature,
        ...
        XrealXbxSession(usbManager, device, model, feature, executor, listener, XrealXbxA01PlusDisplayModeProtocol)
}
