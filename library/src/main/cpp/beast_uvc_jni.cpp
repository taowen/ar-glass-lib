...
void finishFrame(Uvc* u) {
    ...
    bool hasEoi = false;
    for (size_t i = u->assembling.size(); i > 1; --i) {
        if (u->assembling[i - 2] == 0xff && u->assembling[i - 1] == 0xd9) {
            u->assembling.resize(i);
            hasEoi = true;
            break;
        }
    }
...
void consumePayload(Uvc* u, const uint8_t* data, int length) {
    ...
    const bool hasPts = (flags & 0x04) && header >= 6;
    const uint32_t pts = hasPts ? le32(data + 2) : 0;
...
// Locate the MJPEG format and 1920x1080 frame indexes from Beast's VS descriptors.
    ...
    for (int a = 0; a < intf.num_altsetting; ++a) {
        ...
        for (int e = 0; e < setting.bNumEndpoints; ++e) {
            ...
            if (ep.bEndpointAddress == kEndpoint && (ep.bmAttributes & 3) == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) {
                ...
                } else if (!alt && bytes > packetSize) {
                    packetSize = bytes;
                    alt = setting.bAlternateSetting;
                }
            }
        }
        const uint8_t* p = setting.extra;
        int left = setting.extra_length;
        uint8_t mjpeg = 0;
...
bool selectAlt(libusb_device_handle* handle, int desired, int& alt, int& packetSize) {
    ...
    const auto& intf = config->interface[kStreamingInterface];
    for (int a = 0; a < intf.num_altsetting; ++a) {
        const auto& s = intf.altsetting[a];
        for (int e = 0; e < s.bNumEndpoints; ++e) {
            const auto& ep = s.endpoint[e];
            if (ep.bEndpointAddress != kEndpoint || (ep.bmAttributes & 3) != LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) continue;
            int bytes = (ep.wMaxPacketSize & 0x7ff) * (1 + ((ep.wMaxPacketSize >> 11) & 3));
            if (bytes >= desired && (!bestBytes || bytes < bestBytes)) {
                bestBytes = bytes;
                bestAlt = s.bAlternateSetting;
            }
        }
    }
    libusb_free_config_descriptor(config);
    if (!bestAlt) return false;
    alt = bestAlt;
    packetSize = desired;
    return true;
}
...
extern "C" JNIEXPORT jlong JNICALL Java_com_taowen_arglass_BeastCameraNative_start(JNIEnv*, jobject, jint fd) {
    ...
    if (libusb_init(&u->context) != 0 || libusb_wrap_sys_device(u->context, fd, &u->handle) != 0) goto fail;
    {
        ...
        if (libusb_get_device_descriptor(libusb_get_device(u->handle), &descriptor) == 0) {
            u->vid = descriptor.idVendor;
            u->pid = descriptor.idProduct;
        }
    }
    ...
    for (int n = 0; n < kTransfers; ++n) {
        ...
        libusb_fill_iso_transfer(
            ...
            kEndpoint,
            buffer,
            u->packetSize * kPacketsPerTransfer,
            kPacketsPerTransfer,
            isoCallback,
            u,
            1000);
    ...
    u->eventThread = std::thread([u] {
        while (u->running) {
            timeval tv{0, 100000};
            libusb_handle_events_timeout_completed(u->context, &tv, nullptr);
        }
    });
...
fail_running:
    ...
    for (auto* t : u->transfers) {
        delete[] t->buffer;
        libusb_free_transfer(t);
    }
    u->transfers.clear();
fail_claim:
    libusb_release_interface(u->handle, kStreamingInterface);
fail:
...
extern "C" JNIEXPORT jbyteArray JNICALL Java_com_taowen_arglass_BeastCameraNative_readFrame(JNIEnv* env, jobject, jlong p) {
    auto* u = from(p);
    if (!u) return nullptr;
    std::unique_lock<std::mutex> lock(u->frameMutex);
    u->frameReady.wait_for(lock, std::chrono::milliseconds(1200), [u] { return !u->frames.empty() || !u->running; });
    if (u->frames.empty()) return nullptr;
    auto frame = std::move(u->frames.front());
    u->frames.pop_front();
    lock.unlock();
    auto out = env->NewByteArray(frame.size());
    env->SetByteArrayRegion(out, 0, frame.size(), reinterpret_cast<jbyte*>(frame.data()));
    return out;
}
...
extern "C" JNIEXPORT void JNICALL Java_com_taowen_arglass_BeastCameraNative_stop(JNIEnv*, jobject, jlong p) {
    ...
    libusb_set_interface_alt_setting(u->handle, kStreamingInterface, 0);
    libusb_release_interface(u->handle, kStreamingInterface);
    libusb_close(u->handle);
    libusb_exit(u->context);
    u->frameReady.notify_all();
    delete u;
}
